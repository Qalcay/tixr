...
const PLOT_ASPECT_RATIO: f32 = 2.76;
...
impl EconomyEngine {
    ...
    fn step(&mut self) {
        ...
        self.flow = match (self.flow, flowing) {
            (FlowStates::TradingZen, r)     if r <  0.01  => FlowStates::TradingBliss,
            (FlowStates::TradingBliss, r)   if r <  0.002 => FlowStates::TradingTumult,
            (FlowStates::TradingTumult, r)  if r <  0.005 => FlowStates::TradingChaos,
            (FlowStates::TradingChaos, r)   if r <  0.08  => FlowStates::TradingTumult,
            (FlowStates::TradingTumult, r)  if r <  0.04  => FlowStates::TradingBliss,
            (FlowStates::TradingBliss, r)   if r <  0.01  => FlowStates::TradingZen,
            _ => self.flow,
        };
...
fn sim_thread(
    ...
    let tick_rate_factor = Duration::from_millis(66); // 16 = ~62.5 t/s
...
fn main() -> eframe::Result<()> {
    let seed = SeedMarketTheory { qlo: 0x0A01_0F02_0004_DCBA, qhi: 0xFFFD_FFFC_FFFB_FFFA };
    //let seed = SeedMarketTheory { qlo: 0x0987_0987_0987_0987, qhi: 0xFFFC_FFFB_FFFA_FFF0 };
